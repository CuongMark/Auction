...
class Magestore_Auction_Model_Event {
    ...
    // public function insertAuctionBlock($observer){
        // $_block = $observer->getBlock();
        // $_type = $_block->getType();
        // if ($_type == 'catalog/product_price'&&Mage::app()->getRequest()->getControllerName()!='product') {
            // $_child = clone $_block;
            // $_child->setType('auction/list');
            // $_block->setChild('child', $_child);
            // $_block->setTemplate('auction/countdown.phtml');
        // }
    // }
    public function catalog_product_collection_apply_limitations_before($observer) {
        if (!Mage::registry('load_list_auction')) {
            ...
            $productCollection->addFieldToFilter('entity_id', array('in' => $Ids = array_unique(Mage::helper('auction')->getProductAuctionIds(Mage::app()->getStore()->getId()))));
            return $this;
        }
    }
	
	public function sortposition($observer){
		if ($this->getlink() != 'auctionindexindexauction')
			return;
//		if(Mage::app()->getRequest()->getParam('order')=='time'||Mage::app()->getRequest()->getParam('order')==null){
//			if (!Mage::registry('is_join_position')) {
//				Mage::register('is_join_position','1');
//				if(!Mage::app()->getRequest()->getParam('order')) {
//					$installer =  new Mage_Core_Model_Resource_Setup();
//					$observer->getCollection()->getSelect()
//						->joinLeft(array('auction_product'=>$installer->getTable('auction/productauction')),"e.entity_id = auction_product.product_id",array('auction_product.end_date','auction_product.end_time','auction_product.status'))
//						->columns('ADDTIME(DATE_FORMAT(end_date,GET_FORMAT(DATETIME,"ISO")),end_time) AS auction_deadline')
//						->where('auction_product.status = 4')
//						->reset(Zend_Db_Select::ORDER)
//						->order('auction_deadline ASC');
//				}
//			}
//		}
		return $observer->getCollection();
	}
    ...
    public function customer_login($observer) {
        ...
        if ($backUrl) {
            Mage::getSingleton('customer/session')->setBeforeAuthUrl(Mage::getUrl('auction/index'));
