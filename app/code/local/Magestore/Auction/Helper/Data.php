...
class Magestore_Auction_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getProductAuctionEndTimes($store_id = 0, $featured = null) {
        $IDs = array();
        $productIds = array();
        $collection = Mage::getModel('auction/productauction')->getCollection()
                ->addFieldToFilter('status', array('in' => array(3,4,5)));
        if ($featured) {
        ...
        if (count($collection)) {
            foreach ($collection as $item) {
                $productIds[$item->getProductId()] =
                        array('end_time'=>$item->getData('end_date').' '.$item->getData('end_time'),
                            'start_time'=>$item->getData('start_date').' '.$item->getData('start_time'),
                            'price'=> Mage::helper('core')->currency($item->getCurrentPrice(), true, false),
                            'bidder'=>$item->getLastBid()->getData('bidder_name'),
    ...
    public function getProductAuctionFinishedIds(){
        $collection = Mage::getModel('auction/productauction')->getCollection()
    ...
    public function updateBisStatus($auction_id) {
        ...
        if (!count($collection))
            return;
		$winPrice = $collection->getFirstItem()->getPrice();
        $is_winner = true;
        $email = array();
        $i = 0;
        $isSent = false;
        foreach ($collection as $item) {
            if (!in_array($item->getCustomerEmail(), $email) && $i < $auction->getMultiWinner()) {
                ...
                if ($item->getPrice() >= $auction->getReservedPrice()) {					$item->setPrice($winPrice);
                    $isSent = true;
                    $status = 5; //Winner
