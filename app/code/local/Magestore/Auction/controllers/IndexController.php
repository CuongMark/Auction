...
class Magestore_Auction_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function listAuctionInfoAction() {
        $this->getResponse()->setBody(Mage::helper('core')->jsonEncode(Mage::helper('auction')->getProductAuctionEndTimes($store_id, true)));
    }
    public function getNowTimeServerAction() {
        $this->getResponse()->setBody(date("Y/m/d H:i:s", Mage::getModel('core/date')->timestamp(time())));
    }
    public function updateInfoAction() {
        if($auctionId=Mage::app()->getRequest()->getParam('auctionid')){
            $auctionInfo = array();
            ...
            $price_codition = ($max_next_price)?$this->__('(Enter an amount from %s to %s.)',Mage::helper('core')->currency($min_next_price),Mage::helper('core')->currency($max_next_price)):$this->__('(Enter more than %s.)',Mage::helper('core')->currency($min_next_price));
            $auctionInfo = array('end_time'     =>$auction->getData('end_date').' '.$auction->getData('end_time'),
                                'start_time'    =>$auction->getData('start_date').' '.$auction->getData('start_time'),
                                'price'         => Mage::helper('core')->currency($auction->getCurrentPrice(), true, false),
                                'bidder'        =>$auction->getLastBid()->getData('bidder_name'),
                                'update'        =>$auction->getData('update_time'),
                                'end_time'      =>Mage::helper('core')->formatDate(new Zend_Date($auction->getEndDate().' '.$auction->getEndTime(),null, 'en_GB'),'medium',true),
                                'start_time'    =>Mage::helper('core')->formatDate(new Zend_Date($auction->getStartDate().' '.$auction->getStartTime(),null, 'en_GB'),'medium',true),
                                'initPrice'     =>Mage::helper('core')->currency($auction->getData('init_price')),
                                'totalBid'      =>$auction->getTotalBid(),
                                'minNextPrice'  =>$min_next_price,
                                'maxNextPrice'  =>$max_next_price,
                                'price_codition'=>$price_codition,
                                'status'        =>$auction->getData('status'),
                           );
            $this->getResponse()->setBody(Mage::helper('core')->jsonEncode($auctionInfo));
        }
    }
    ...
    public function checkoutAction() {
        ...
        try {
            ...
            Mage::getSingleton('checkout/session')->addSuccess(Mage::helper('checkout')->__('The auctioned product %s has been added to your cart successfully at your winning price.', $product->getName()));
            $this->_redirect('checkout/cart', array());
        } catch (Exception $e) {
    ...
    public function changewatcherAction() {
        ...
        if (!Mage::getSingleton('customer/session')->isLoggedIn()) {
            if (isset($_SERVER['HTTP_REFERER'])) {
				...
				$backUrl = Mage::getSingleton('core/session')->getLastUrl();
    ...
    public function bidAction() {
        ...
        if (!$customerSession->isLoggedIn()) {
            ...
            $this->getResponse()->setBody(Mage::helper('core')->jsonEncode(array('success'=>false,'msg'=>$_helper->__('You have to log in to bid.'))));
            return;
        }
        ...
        if (!isset($data['price']) || !$data['price']) {
            $this->getResponse()->setBody(Mage::helper('core')->jsonEncode(array('success'=>false,'msg'=>$_helper->__('Your bid price is invalid.'))));
            return;
        }
        ...
        if (!$customer->getBidderName()) {
            if ($bidderNameType == '2') {
                ...
                $this->getResponse()->setBody(Mage::helper('core')->jsonEncode(array('success'=>false,'msg'=>$_helper->__('You have to create a bidder name before bidding for auctioned products.'))));
                return;
            }
            if ($bidderNameType == '3') {
                if (!$customer->getBidderName()) {
        ...
        if ($auction->getStatus() == 5) { //complete auction
            $this->getResponse()->setBody(Mage::helper('core')->jsonEncode(array('success'=>false,'msg'=>$_helper->__('Completed Auction'))));
            return;
        }
        ...
        if ($bidType) {
            ...
            if ($customer->getId() == $lastAuctionBid->getCustomerId()) {
                $this->getResponse()->setBody(Mage::helper('core')->jsonEncode(array('success'=>false,'msg'=>$_helper->__('You have placed the highest bid.'))));
                return;
            }
            ...
            try {
				...
				$this->getResponse()->setBody(Mage::helper('core')->jsonEncode(array('success'=>true,'msg'=>$_helper->__('You have placed a bid.'))));
                $check = true;
            } catch (Exception $e) {
                $this->getResponse()->setBody(Mage::helper('core')->jsonEncode(array('success'=>false,'msg'=>$e->getMessage())));
            }
        } else {
            ...
            if ($check_autobid == 1) {
                ...
                $autobid->setData($data)
						->setId($autobidid)
                        ->setStoreId($store_id);
				// Zend_debug::dump($autobid->getData());exit;
                try {
                    $autobid->save();	
					$count = Mage::getModel('auction/auction')->getCollection()                    						
						->addFieldToFilter('productauction_id', $auction->getId())->getSize();						
					if(!$count){								
						$product = Mage::getModel('catalog/product')->load($auction->getProductId());											
						$timestamp = Mage::getModel('core/date')->timestamp(time());											
						$auctionbid = Mage::getModel('auction/auction');											
						$data['price'] = $auction->getMinNextPrice();											
						...
						->setStoreId($store_id);											
						$auctionbid->save();							
						$this->getResponse()->setBody(Mage::helper('core')->jsonEncode(array('success'=>true,'msg'=>$_helper->__('You have a successful auto bid'))));
						$autobid->emailToBidder();							
						return;									
					}	
					$autobid->emailToBidder();
                    $check = true;
                    $this->getResponse()->setBody(Mage::helper('core')->jsonEncode(array('success'=>true,'msg'=>$_helper->__('You have placed an auto bid successfully.'))));
                } catch (Exception $e) {
                    $this->getResponse()->setBody(Mage::helper('core')->jsonEncode(array('success'=>false,'msg'=>$e->getMessage())));
                }
				...
				$check = false;
                $this->getResponse()->setBody(Mage::helper('core')->jsonEncode(array('success'=>false,'msg'=>$_helper->__('You have already placed an auto bid for this auction.'))));
                return;
            }
        }
        if ($check || $check == true) {
            $this->autobid($auction,$customer,$bidType);            
        }
    }
    
    public function autobid($auction,$currentCustomer,$bidType){				
		$_helper = Mage::helper('auction');		
        $price = $auction->getMinNextPrice();
        $autobid = Mage::getModel('auction/autobid')->getCollection()
                    ->addFieldToFilter('productauction_id', $auction->getId())
                    ->addFieldToFilter('price', array('gteq' => $price))
                    ->setPageSize(2)
                    ->setCurPage(1);
        $autobid->getSelect()->order('price DESC');
        if($autobid->getSize()==1){
            $autobid = $autobid->getFirstItem();
			...
			if($currentCustomer->getId()!=$customer->getId())			
				$this->getResponse()->setBody(Mage::helper('core')->jsonEncode(array('success'=>true,'msg'=>$_helper->__('you were outbid by auto bid'))));
        }elseif($autobid->getSize()>1){
            $autobid1 = $autobid->getLastItem();
            $product = Mage::getModel('catalog/product')->load($auction->getProductId());
            $timestamp = Mage::getModel('core/date')->timestamp(time());
            $auctionbid = Mage::getModel('auction/auction');
            $customer = Mage::getModel('customer/customer')->load($autobid1->getCustomerId());
            $price = $autobid1->getPrice();
            $data['price'] = $price;
            $data['product_id'] = $auction->getProductId();
            $data['productauction_id'] = $auction->getId();
            $data['customer_id'] = $customer->getId();
            ...
            $auctionbid->setData($data)
                    ->setStoreId($store_id);
            $auctionbid->save();
            $autobid2 = $autobid->getFirstItem();
            $auctionbid = Mage::getModel('auction/auction');
            $customer = Mage::getModel('customer/customer')->load($autobid2->getCustomerId());
            $data['price'] = $price+$auction->getData('min_interval_price');
            $data['product_id'] = $auction->getProductId();
            $data['productauction_id'] = $auction->getId();
            $data['customer_id'] = $customer->getId();
            $data['customer_name'] = $customer->getName();
            ...
            $auctionbid->save();
			if($currentCustomer->getId()!=$customer->getId())				
				$this->getResponse()->setBody(Mage::helper('core')->jsonEncode(array('success'=>false,'msg'=>$_helper->__('you were outbid by auto bid'))));
        }		
    }
    
    public function testAction(){
        $auction = Mage::getModel('auction/productauction')->loadAuctionByProductId(553);
        $customer = Mage::getModel('customer/customer')->load(143);
        $this->autobid($auction,$customer);
    }
